<script setup>
import { ref, computed, onMounted, onBeforeUnmount } from 'vue'
import { attractionImageSrc, handleImageError } from '@/utils/Weatherhelpers'
import { useAttractionImageStore } from '@/stores/Attractionimagestore'

// 갤러리처럼 관광지가 40개씩 한 화면에 v-for로 쭉 나오는 경우 마운트되자마자
// 전부 Unsplash를 검색하면(WeatherCard/RecommendedAttraction과 달리) Demo 등급
// rate limit(시간당 50회)을 순식간에 다 써버림
// 그래서 실제로 스크롤돼서 화면에 보이는 카드만 IntersectionObserver로 감지해서 그때그때 검색
const props = defineProps({
  name: { type: String, required: true },
  cityName: { type: String, default: '' }, // 추가
  imageFileName: { type: String, required: true },
  alt: { type: String, default: '' },
})

onMounted(() => {
  observer = new IntersectionObserver(
    (entries) => {
      if (entries[0].isIntersecting) {
        attractionImageStore.fetchImage(props.name, props.cityName)
        observer.disconnect()
      }
    },
    { rootMargin: '200px' },
  )
  if (imgRef.value) observer.observe(imgRef.value)
})

const attractionImageStore = useAttractionImageStore()
const imgRef = ref(null)
let observer = null

onBeforeUnmount(() => {
  observer?.disconnect()
})

// 캐시에 이미 있으면(다른 페이지에서 먼저 검색된 관광지 등) observer 여부와 무관하게 바로 반영됨
const attractionPhoto = computed(() => attractionImageStore.getImageByQuery(props.name))
// const src = computed(() => attractionPhoto.value?.url ?? attractionImageSrc(props.imageFileName))
const src = computed(() => {
  const result = attractionPhoto.value?.url ?? attractionImageSrc(props.imageFileName)
  console.log(
    '[thumb]',
    props.name,
    '→ photo:',
    attractionPhoto.value?.url,
    '| fallback:',
    attractionImageSrc(props.imageFileName),
    '| 최종:',
    result,
  )
  return result
})
</script>

<template>
  <div class="lazy-thumb-wrap">
    <img
      ref="imgRef"
      class="lazy-thumb"
      :src="src"
      :alt="alt || name"
      @error="handleImageError($event, imageFileName)"
    />
  </div>
</template>

<style scoped>
.lazy-thumb-wrap {
  position: relative;
  overflow: hidden;
}
.lazy-thumb {
  position: absolute;
  inset: 0;
  width: 100%;
  height: 100%;
  object-fit: cover;
  display: block;
}
.unsplash-credit {
  position: absolute;
  right: 6px;
  bottom: 6px;
  font-size: 10px;
  color: #ffffff;
  background: rgba(16, 24, 48, 0.55);
  padding: 2px 6px;
  border-radius: 999px;
  text-decoration: none;
}
.unsplash-credit:hover {
  background: rgba(16, 24, 48, 0.75);
}
</style>
