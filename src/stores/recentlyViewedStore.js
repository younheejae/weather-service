...
// 도시 객체 전체가 아니라 도시 id만 저장해두고 화면에 보여줄 때
// weatherStore(실시간 API 데이터가 저장된 전역 스토어)에서 최신 정보를 다시 조회함
export const useRecentlyViewedStore = defineStore('recentlyViewed', () => {
  // state: 최근 본 순서대로 id를 저장 (index 0 = 가장 최근)
  const cityIds = ref([])

  // getter: id 배열을 실제 도시 객체(실시간 날씨 포함) 배열로 변환해서 제공.
  const recentCities = computed(() => {
    const weatherStore = useWeatherStore()
    return cityIds.value.map((id) => weatherStore.getCityById(id)).filter(Boolean)
  })
